...
#endif
    std::cout << "[OverlayToast] Modern DX11 Zero-Lag Engine initialized." << std::endl;
}
...
void OverlayToast::OverlayThreadProc() {
    ...
    if (m_hWnd) {
        ...
        if (InitDirectX()) {
            std::cout << "[OverlayToast] DX11 Overlay initialized successfully." << std::endl;
        } else {
            std::cerr << "[OverlayToast] Failed to initialize DX11 Overlay!" << std::endl;
        }
    }
...
bool OverlayToast::InitDirectX() {
    ...
    swapChainDesc.AlphaMode = DXGI_ALPHA_MODE_PREMULTIPLIED; // Essential for transparent layered windows!
    swapChainDesc.Flags = 0;

    hr = dxgiFactory->CreateSwapChainForHwnd(m_d3dDevice, m_hWnd, &swapChainDesc, nullptr, nullptr, &m_swapChain);
...
void OverlayToast::RenderDX() {
    ...
    m_swapChain->Present(0, 0); // DXGI_SWAP_EFFECT_FLIP_DISCARD requires Present(0, 0) for lowest latency
}
